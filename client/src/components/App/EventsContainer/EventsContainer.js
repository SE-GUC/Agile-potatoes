...
class EventsContainer extends Component {
  ...
  getAllEvents = async ()=>{
    try {
      ...
      this.setState({toggle:0 ,events:[...allEvents.data]});
     } catch (error) {
  ...
  getRecommendedEvents = async ()=>{
    try {
      ...
      this.setState({toggle:0,events:[...recommendEventsRes.data]});
     } catch (error) {
      console.log('GOT ERROR')
    }
  }
  getPartnerEvents= async()=>{
    try {
      ...
      this.setState({toggle:0 ,events:[...events.data]});
     } catch (error) {
  ...
  getPendingEventsAdmin= async()=> {
    ...
    this.setState({toggle:0 ,events:[...response.data]});
    } catch (error) {
      console.log('GOT ERROR ' + error)
    }
  }
        ...
        <div className='row'>
            <div className='side-bar col-sm-2 ' >
                <div className="list-group">
                  ...
                  {
                    JSON.parse(localStorage.getItem('token')) && (JSON.parse(localStorage.getItem('token')).data.userData.userType === 'Partner' || JSON.parse(localStorage.getItem('token')).data.userData.userType === 'Admin')
                    &&
