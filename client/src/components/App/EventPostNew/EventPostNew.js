...
class EventPostNew extends Component {
 ...
 this.handleChange = this.handleChange.bind(this);
  ...
  async onClickApprove(){
    ...
    await axios.put(
      "http://localhost:3001/api/event/5cb0886e1c9d440000b79da0/approve",
      {
        "userType" : "Admin"
      }
    );
  ...
  async onClickSubmit (){
   ...
   const resp=  await axios.put(
      "http://localhost:3001/api/event/5cb0886e1c9d440000b79da0"
...
console.log(e.target.value)
  ...
  async componentDidMount() {
    ...
    let event = await axios.get(
      "http://localhost:3001/api/event/Post/5cb0886e1c9d440000b79da0"
    );
  ...
  getCommentsSorted() {
    ...
    return this.state.eventData.commentsByAdmin.concat(
      this.state.eventData.commentsByPartner
    );
  }
  ...
  async checkIfAlreadyBooked() {
    ...
    for (let i = 0; i < attendees.length; i++) {
      if (attendees[i]._id === this.state.userData._id) booked = true;
    }
    if (booked) {
      this.setState({
        userHasBooked: true
  ...
  onClickBook = e => {
    ...
    axios
      .put(
        "http://localhost:3001/api/event/5cab5d3222a833137c7acd38/attending",
        {
          userID: this.state.userData._id,
          userType: this.state.userData.userType
        }
  ...
  onClickCancel = e => {
    ...
    axios
      .put(
        "http://localhost:3001/api/event/5cab5d3222a833137c7acd38/notAttending",
        {
          userID: this.state.userData._id,
          userType: this.state.userData.userType
        }
      ...
      return (
        <div className="event-post offset-sm-2 col-sm-8 row ">
          <div className="left-of-post col-sm-9">
              ...
              <br></br>
              <button className="btn btn-success ctrl-button col-sm-12 " onClick={()=>this.onClickSubmit()}>
                    Done
                  </button>
          </div>
        </div>
          ...
          <div className="right-of-post col-sm-3">
            ...
            {this.state.userHasBooked ? (
              <button
                ...
                disabled={this.state.eventData.remainingPlaces <= 0}
              ...
              <button
                ...
                disabled={
                  ...
                  this.state.eventData.remainingPlaces <= 0
                }
            ...
            <p className="text-muted text-center">
              remaining seats:{this.state.eventData.remainingPlaces}
            </p>
            {this.state.userData.userType === "Admin" &&
              this.state.eventData.eventStatus === "Submitted" && (
                <div>
            ...
            {this.state.userData.userType === "Partner" &&
              ...
              (
                <div>
                  ...
                  <br />
                  <br />
                  <button className="btn btn-success ctrl-button col-sm-12 " onClick ={()=>this.setState({Edit:true})}>
                    Edit
                  </button>
                </div>
              )}
              ...
              this.state.eventData.eventStatus === "Approved" && (
                <div>
                  ...
                  <button
                    ...
                    className="btn btn-warning ctrl-button col-sm-12 "
                  >
                    Close Event
                  </button>
                </div>
            ...
            {(this.state.userData.userType === "Partner" ||
              ...
              this.state.eventData.eventStatus === "Finished" && (
                <div>
                  ...
                  <br />
                  <button
                    onClick={this.reOpenEvent.bind(this)}
            ...
            <div
              ref="alert"
              className="alert alert-danger alert-dev"
              role="alert"
            >
